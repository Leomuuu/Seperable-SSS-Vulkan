...
namespace VlkEngine {
	...
	void RenderDescriptor::CreateDescriptorPool()
	{
	...
	void RenderDescriptor::CreateDescriptorSets(RenderBuffer* renderBuffer, RenderImage* renderImage)
	{
		...
		for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
			...
			bufferInfo.buffer = renderBuffer->uniformBuffers[i];
			bufferInfo.offset = 0;
			bufferInfo.range = sizeof(MVPMatrix);

			VkDescriptorImageInfo imageInfo{};
			imageInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
			imageInfo.imageView = renderImage->textureImageView;
			imageInfo.sampler = renderImage->textureSampler;

			VkDescriptorBufferInfo fragbufferInfo{};
			fragbufferInfo.buffer = renderBuffer->fraguniformBuffers[i];
